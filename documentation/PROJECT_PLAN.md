...
| Persistent level caps | Complete; manually verified | New saves begin at level 5 and story-trainer victories raise the saved cap according to `LEVEL_CAPS.md`. |
| Level to Cap | Intermediate-presentation reduction implemented; build and manual verification pending | The party action advances one level at a time, suppresses intermediate level/stat announcements, shows the full native presentation for the level that reaches the cap, and preserves every move and evolution prompt. |
| Bait encounters | Complete; manually verified | Poké Bait and Shiny Bait work on valid terrain; invalid use is rejected; preserved encounter types remain available. Random non–Shiny Bait encounters are forced non-shiny, while explicitly shiny scripted encounters such as the Lake of Rage Gyarados are preserved. See `BAIT_ENCOUNTERS.md`. |
| Non-battle roadblock Pokémon | Implemented; build and manual verification pending | Sudowoodo and Snorlax now flee after their existing activation dialogue instead of starting wild battles. Their normal completion, visibility, and no-respawn state is preserved so Route 36 and Diglett’s Cave progression still opens permanently. |
...
| Talk-initiated trainer battles | Complete; manually verified | `DISABLE_TRAINER_LINE_OF_SIGHT` prevents automatic trainer detection before it takes field control. The shared undefeated talk path displays a generated species/reward offer and asks whether to battle. Custom map scripts, automatic battles, defeated dialogue, and rematches remain unchanged. |
| Trainer victory rewards | Shared-trainer infrastructure complete; manually verified | `data/trainer_rewards.csv` configures one first-victory item and quantity per shared talk trainer. The existing defeated path prevents the initial reward from repeating; current rematches and custom or automatic battle scripts do not use this table. Additional reward mappings can be added during trainer-content work. |
| Summary friendship value | Complete; manually verified | The Skills page displays the exact `0-255` friendship value for party and boxed Pokémon without adding save state or another Summary hook. Eggs retain the normal heading because the same field stores hatch cycles. |
| In-battle player item restriction | Complete; manually verified | `DISABLE_ITEMS_IN_TRAINER_BATTLE` rejects active player item selections in trainer battles and returns to command selection. Wild-battle Bag behavior and held items remain unchanged; revisit stricter UI or pocket filtering only if playtesting requires it. |
...
| Trainer AI changes | Complete; accepted after gameplay evaluation | Expert singles trainers use the resident exact-information, action-blind evaluator with tactical move/switch comparison, concrete player-action prediction, adaptation, post-KO selection and original-HGSS fallback. Doubles and unsupported formats remain on original HGSS AI. Gameplay evaluation found that the AI could initially be manipulated in some sequences but subsequently recognized and countered the pattern; this is accepted for the current release. Reopen only if playtesting identifies a repeatable manipulation that remains reliably exploitable after adaptation. Optional hardening includes preventing a Ghost-type user from selecting Curse when the current target is already cursed; prefer the minimal generated effect-class guard and retain the enforced 500-byte overlay-130 reserve before considering full Curse valuation. Remaining semantics-coverage entries are not release blockers. See `TRAINER_AI_TECHNICAL_DESIGN.md`, `TRAINER_AI_SEMANTICS_COVERAGE.md` and `TRAINER_AI_LEGACY_ROLLBACK_CHANGELIST.md`. |
...
speech through the existing No Info Needed path instead of presenting the
Control Info / Adventure Info menu.
