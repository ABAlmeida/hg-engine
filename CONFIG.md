...
### ``armips\include\config.s`` and ``include\config.h``
 ...
 ``IMPLEMENT_LEGENDARY_SANCTUARY`` replaces the Bug-Catching Contest lifecycle with the permit-driven Legendary Sanctuary. ``SANCTUARY_TIME_LIMIT_MINUTES``, ``SANCTUARY_BALL_COUNT``, ``SANCTUARY_BALL_CATCH_RATE_PERCENT``, and ``SANCTUARY_ENCOUNTER_RATE`` control the visit duration, supplied Balls, and active encounter behavior. Four replacement pools are selected by ``VAR_SANCTUARY_POOL_STAGE``. Keep ``SANCTUARY_CONTENT_READY`` false until at least one pool is populated; admission is then refused without consuming a Sanctuary Permit. When ``DEBUG_CHEATS`` is enabled, speaking to the admission attendant automatically sets the story unlock and awards one Permit if the Bag currently has none. See ``documentation/LEGENDARY_SANCTUARY_PLAN.md``.
