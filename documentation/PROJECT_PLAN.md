...
| Public demo boundary | Implemented; build and manual verification pending | `HEARTLESS_GOLD_DEMO` is enabled in `include/config.h`. After Bugsy's successful reward sequence—or its Bag-full branch—the game displays the demo-complete message and uses the verified field-safe reset path to return to normal startup. Commenting out the option restores progression beyond Azalea Gym. |
