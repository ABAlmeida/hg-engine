...
| --- | --- | ---: | --- |
| Level to Cap | Party-menu state/flow: previously 656; Pokémon one-level and capped-level routines: 280 | At least 936 before the pending UI reduction measurement | This is one of the largest Heartless Gold additions. Intermediate levels now enter the existing move/evolution loop directly and reserve the native level/stat presentation for the final level. Post-evolution continuation reuses the existing window-34 buffer and native frame renderer; it adds no BSS, heap, save, archive, or VRAM allocation. Measure the updated `party_menu.o` and remaining overlay-129 headroom on the next explicitly requested build. |
...
| Cross-region Fly | Recompressed in-place overlay-101 patch | 0 synthetic-overlay bytes | Replaces the four-byte entry of the existing current-region gate. Destination Flypoint flags remain required. The build recompresses overlay 101 after patching, so runtime allocation, heap, save, stack, and VRAM are unchanged and packaged ROM size remains effectively size-neutral. |
