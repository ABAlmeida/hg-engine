...
| Bait encounters | Complete; natural-shiny restriction pending build and manual verification | Poké Bait and Shiny Bait work on valid terrain; invalid use is rejected; preserved encounter types remain available. Non–Shiny Bait wild encounters are forced non-shiny so Shiny Bait is the sole wild shiny source. See `BAIT_ENCOUNTERS.md`. |
