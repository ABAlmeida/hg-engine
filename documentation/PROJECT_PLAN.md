...
| Trainer AI changes | Complete; accepted after gameplay evaluation | Expert singles trainers use the resident exact-information, action-blind evaluator with tactical move/switch comparison, concrete player-action prediction, adaptation, post-KO selection and original-HGSS fallback. Doubles and unsupported formats remain on original HGSS AI. Gameplay evaluation found that the AI could initially be manipulated in some sequences but subsequently recognized and countered the pattern; this is accepted for the current release. Reopen only if playtesting identifies a repeatable manipulation that remains reliably exploitable after adaptation. Remaining semantics-coverage entries are optional hardening rather than release blockers. See `TRAINER_AI_TECHNICAL_DESIGN.md`, `TRAINER_AI_SEMANTICS_COVERAGE.md` and `TRAINER_AI_LEGACY_ROLLBACK_CHANGELIST.md`. |
